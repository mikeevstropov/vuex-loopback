...
Vuex module created by module factory has [the state](#state) to interact with a single document and a document list. The following fields will contain data when you *get*, *create* or *update* a single item.
...
At previous step we provided the model with default fields to the module factory. An action `CREATE_TEMP_ITEM` will create a new item by this model automatically (only `tempItem` state, not in database).
